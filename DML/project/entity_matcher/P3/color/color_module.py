...
def extract_attribute_val(product, attr_1, attr_2, product_info):
     ...
     try:
          ...
          try:
               desp = str(product_desp).lower()
               desp_words_list = desp.split(' ')
               
               
               extracted_attr_value = list(set(colors_dict) & set(desp_words_list))
               if len(extracted_attr_value) > 0:
                    #print "color extraction works"
                    pass
               extracted_colors.extend(extracted_attr_value)
               extracted_attr_values.extend(extracted_attr_value)
          except:
               print "error : color extraction failed"
...
def extract_attribute_value(product, attr_1, attr_2):
     ...
     return list(set(extracted_attr_values))
...
def find_match(json_product_1, json_product_2, dictionary = "files/colors_dict.txt", attr_1 = "Color", attr_2 = "Actual Color"):
     ...
     try:
          ...
          try:
               if colors_dict_loaded == False:
                    get_colors_from_dict(dictionary)
                    colors_dict_loaded = True
          except:
          ...
          #getting color attribute value for product 2
          (p2_attr_1_values,p1_attr_1_found)  = get_attribute_value(p2,attr_1)
          
          #getting actual color attribute value for product 2
          (p2_attr_2_values,p2_attr_2_found)  = get_attribute_value(p2,attr_2)
          
          p2_attr_values = p2_attr_1_values + p2_attr_2_values
          
          
          #extracting color attribute values for product 2 if color or actual color attributes are not available/valid
          ...
          common_attr_values = list(set(p1_attr_values) & set(p2_attr_values))
          if len(common_attr_values) > 0:
               return [1,1]
          ...
          prod_str = str(p2).lower()
          prod_words_list = prod_str.split(' ')          
          common_attr_values = list(set(p1_attr_values) & set(prod_words_list))
          if len(common_attr_values) > 0:
               return [1,1]          
          ...
          prod_str = str(p1).lower()
          prod_words_list = prod_str.split(' ')          
          common_attr_values = list(set(p2_attr_values) & set(prod_words_list))
          if len(common_attr_values) > 0:
               return [1,1]                    
          ...
          prod_1_str = str(p1).lower()
          prod_1_words_list = prod_1_str.split(' ')                    
          prod_2_str = str(p2).lower()
          prod_2_words_list = prod_2_str.split(' ')                              

          common_attr_values = list(set(colors_dict) & set(prod_1_words_list + prod_2_words_list))
          if len(common_attr_values) > 0:
               return [1,1]                              
