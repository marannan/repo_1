...
def extract_json():
    ...
    for i in xrange(10):
        ...
        if str(sort_attr[i][0]) == "product name" or str(sort_attr[i][0]) == "product long description" or str(sort_attr[i][0]) == "product short description" or str(sort_attr[i][0]) == "brand" or str(sort_attr[i][0]) == "gtin" or str(sort_attr[i][0]) == "upc" or str(sort_attr[i][0]) == "country of origin: components":
            ...
            num_bins = 500
            fig = plt.figure(i)
            plt.hist(len_list,bins = 500, color='blue');
            plt.title( str(sort_attr[i][0]))
            plt.xlabel('Length of "' + str(sort_attr[i][0]) + '"')
            plt.ylabel('Number of Products')
            plt.grid(True)
            fig.show()
            fig.savefig(str(sort_attr[i][0])+'.png')
            raw_input()
            a = 10
            #n,bins,patches = plt.hist(len_list, num_bins, normed=0, facecolor='green', alpha=0.8)
            #plt.plot(bins, n, 'r--')
            #print "Type: textual"
