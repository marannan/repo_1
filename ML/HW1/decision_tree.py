...
def numeric_attributes_split(data, attribute, class_label):
    labels_list = []
    values = []
    ...
    return (minimum_label, minimum_entropy)


def build_decision_tree(data, level, attributes_list, attributes_label, class_label, m=1):

    (minimum_entropy_attribute, minimum_entropy, threshold)= find_best_candidate_split(data, attributes_list, attributes_label, class_label)

    child = decision_tree(minimum_entropy_attribute, threshold, [], attributes_list, attributes_label, class_label)

    if attributes_list.has_key(minimum_entropy_attribute) == False: #numeric
...
def find_best_candidate_split(data, attributes_list, attributes_label, class_label):
    ...
    for attribute in attributes_label:
        if attributes_list.has_key(attribute) == False: #if numeric
            (threshold_value, entropy_value) = numeric_attributes_split(data, attribute, class_label)
    ...
    return (minimum_entropy_attribute, minimum_entropy, threshold)
...
class decision_tree(object):
    ...
    def display_tree(self, level):
        temp_str_1 = "".join([tab_space for _ in range(level)]) + self.attribute_name
        return_str = ""
        if self.attributes_list.has_key(self.attribute_name) == False: #if numeric
            return_str = temp_str_1 + "<=" +str(self.threshold)
            if isinstance(self.class_or_child[0].class_or_child, basestring):
                print "{1}: {0}".format(self.class_or_child[0].class_or_child, return_str)

            else:
                print return_str
                self.class_or_child[0].display_tree(level+1)
            return_str =  temp_str_1 + ">" + str(self.threshold)
            if isinstance(self.class_or_child[1].class_or_child, basestring):
                return_str = return_str +  ": {0}".format(self.class_or_child[1].class_or_child)
                print return_str
            else:
                print return_str
            ...
            for index, child in enumerate(self.class_or_child):
                temp_str_2 = temp_str_1 + "=" + self.attributes_list[self.attribute_name][index]
                if isinstance(self.class_or_child[index].class_or_child, basestring):
                    temp_str_2 = temp_str_2 + ": {0}".format(self.class_or_child[index].class_or_child)
                    print temp_str_2
                else:
                    print temp_str_2
