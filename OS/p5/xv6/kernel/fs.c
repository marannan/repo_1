...
#define FLAG_1 0x00FFFFFF
#define FLAG_2 0x00FFFFFFF
...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      //T_Checked
      bfree(ip->dev, ((ip->addrs[i]) & FLAG_2));
      ip->addrs[i] = 0;
    }
  }
  ...
  if(ip->addrs[NDIRECT]){
    //T_Checked
    bp = bread(ip->dev, (ip->addrs[NDIRECT] & FLAG_2));
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
      if(a[j])
        //T_Checked
        bfree(ip->dev, (a[j] & FLAG_2));
    }
    brelse(bp);
    //T_Checked
    bfree(ip->dev, (ip->addrs[NDIRECT] & FLAG_2));
    ip->addrs[NDIRECT] = 0;
  }
...
stati(struct inode *ip, struct stat *st)
{
  ...
  if(ip->type == T_CHECKED) 
  {
    uchar checksum_1 = NULL;
    uchar checksum_2 = NULL;
    ...
    for(i = 0; i < (NDIRECT+1); i++) 
    {
      ...
      if(i == NDIRECT)
      {
        ...
        {
          checksum_2 ^= (uchar)((a[j] & 0xFF000000) >> 24);
        }
      ...
      else 
      {
        ...
        temp = (temp >> 24);
        checksum_2 =  (uchar) temp;
        checksum_1 ^= checksum_2;
      }
    }
      st->checksum = checksum_1;
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m)
  {
    ...
    if(ip->type == T_CHECKED) 
    {
      ...
      if(bn < NDIRECT) {
        sum_1 = ip->addrs[off/BSIZE] & 0xFF000000;
        sum_1 = (sum_1 >> 24);
      }
