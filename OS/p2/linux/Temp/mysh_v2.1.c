...
void Error_Command()
{
	//check to use STDOUT_ERR or STDOUT_FILENO for printing the error messages
...
void Check_Batch_File()
{
	...
	{
		//Just exit when batch file cant be opened
		//Error_Command();
		exit(1);
	}
...
int My_Shell()
{
	...
	Input_Command_Ptr = inputString(stdin, 10);
    //printf("%s\n", Input_Command_Ptr);

    if((int)strlen(Input_Command_Ptr)*(int)sizeof(char) > 512)
	{
		Input_Command[512] = '\n';
	}
    ...
    if(Batch_Mode == 1)
    {
    	write(STDOUT_FILENO, Input_Command, 512);
    }
	...
	if(Return_Val == 0)
	{
		...
		execvp(Argv[0],Argv);
		Error_Command(); //print error becuase its an error command eg /bogus
		exit(1);
	}
...
int main(int argc, char *argv[])
{
	if(argc > 2)
	{
		//Exit when there are more than two args
		//Error_Command();
		exit(0);
	}
