...
int
isVAS(struct proc *p, uint addr)
{
    if (addr < PGSIZE || (addr >= p->sz && addr < p->ssz) || addr >= USERTOP) {
        return -1;
    }
    return 0;
}
...
{
    if (p->pid != 1) {
        if(isVAS(p, addr) == -1 || isNAS(p, addr+4) == -1)
            return -1;
    }
...
argptr(int n, char **pp, int size)
{
    ...
    if(argint(n, &i) < 0)
        return -1;
    if (isVAS(proc, (uint)i) == -1 || isNAS(proc, (uint)i+size) == -1)
        return -1;
    *pp = (char*)i;
    return 0;
}
