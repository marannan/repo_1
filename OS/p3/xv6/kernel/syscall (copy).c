...
uint is_invalid_addr(struct proc *p, uint addr)
{

  //check if the address is in valid range
  ...
  //check if the address is in first page
  ...
  //check if the address beyond in codesegment but before the start of heap
...
uint is_invalid_addr_2(struct proc *p, uint addr, int sz)
{

  //check if the address is in valid range
  if( ((addr >= p->sz)&&(addr < (USERTOP-p->proc_stack_sz *PGSIZE))) || 
      ((addr+sz > p->sz)&&(addr+sz < (USERTOP-p->proc_stack_sz*PGSIZE)))|| 
      addr==0 || addr >= USERTOP || addr+sz> USERTOP)
    return 1;
  ...
  //check if the address beyond in codesegment but before the start of heap
  if ((addr >= proc->proc_code_sz && addr < (proc->proc_code_sz + PGSIZE)) || 
      ((addr + sz) > proc->proc_code_sz && (addr + sz) < (proc->proc_code_sz + PGSIZE)))
  {
    return 1;
  }
...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if (is_invalid_addr(proc, addr) == 1)
  {
    return -1;
  }
...
argint(int n, int *ip)
{
  //cprintf("argint proc->tf->esp: %d proc->sz: %d\n", proc->tf->esp,proc->sz );
...
argptr(int n, char **pp, int size)
{
  ...
  if (is_invalid_addr_2(proc, i, size) == 1)
  {
      return -1;
  }
